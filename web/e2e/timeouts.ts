...
//
// `180_000` was written out 17 times across seven files. It is not an
// arbitrary generous number: it is "download ~125 MiB from a local static
// server, compile and instantiate the wasm, spawn and barrier up to 8 compute
// workers, then run a first encode" — under this suite's own parallelism (see
// `playwright.config.ts`'s `workers`). Changing it should be one edit.

...
// reply, so codec_init can run only once every byte has actually landed in
// wasm memory — that's what 'finish' is for.
//
...
//      ualloc(modelBytes) up front — the model region must exist before the
//      first chunk can land in it. Reply: { id, r: { ok: true } } (or a Fail
...
//      memory and run `thread_setup`, i.e. once its stack/TLS are actually
//      valid — CLAMPING W to `threads_ready()` if some never arrive (the
//      contract rust/urlcodec's `codec_init` documents), then calls
//      codec_init over the now-complete model + tokenizer,
//      ufree's both scratch regions, and replies { id, r: Info | Fail }. A
//      chunk/tokenizer write that failed earlier is surfaced here (they have
//      no id of their own to reply to). Without this wait, codec_init could
//      hand the first job to a worker that hasn't set its stack yet.
//
...
// spinning — the cleanup path e2e/threads.smoke.spec.ts exercises. A missing
// worker N>1 instead clamps W to N-1 and the tier runs, which is the whole
// point of the clamp.
...
const TOKENIZER_MARGIN = 2 << 20; // 2 MiB — real BPE vocab file is ~500 KiB (models/url-bpe-8k-cap24-s0/tokenizer.json)
 ...
 * the actual artifact instead of a flat guess that a smaller model wastes
 ...
 * lives in the wasm module's data section, and the whole point of the hook is
 * to produce it WITHOUT a real fault. What the client keys on is `code`, which
...
// Which of those actually reported ready, indexed by `id - 1`. The count of
// LEADING trues is what W can be clamped to, because threads.rs picks
// participants by id (`id <= W`): a worker takes part in a job only while its
// id is within the configured W, so a gap — worker 2 dead with 1 and 3 alive
// — cannot be clamped around, only truncated before.
...
self.onmessage = async (e: MessageEvent) => {
  ...
  try {
    if (m.op === 'setup') {
      ...
      // time out otherwise — so the hook is inert on the fallback the recovery
      // loads. That is what stops the E2E from poisoning the replacement too
      // (the query string is still on the URL when it reloads).
      ...
      if (shared) {
        ...
        for (let id = 1; id <= threads; id++) {
          ...
          // downloaded) is what actually awaits `threadReady` — without a
          // handler attached now, a worker that fails fast would reject an
          // unobserved promise and surface first as a console
          // `unhandledrejection`, ahead of and separate from the load's own
          // error reporting.
          ready.catch(() => {});
          ...
          // (id === 1) skips its ready message, so the others really do spin
          // up and register — exercising the "compute workers already
          // spawned and spinning" cleanup path a threads-tier fallback must
          // handle (see client.ts's mtFailHandshakeRequested).
    ...
    if (m.op === 'finish') {
      ...
      // Establish how many compute workers are genuinely ready (stack + TLS
      // valid, and the wasm's own bookkeeping agrees) before codec_init hands
      // out the first job — see the handshake note at the top of this file —
      // and run at that width. Both phases below share one 5s deadline, so
      ...
      if (shared && threads > 0) {
        ...
        // any more — the clamp below decides what is usable. Rejections are
        ...
        // which is what codec_init's dispatch actually relies on rather than
        ...
        // CLAMP, don't throw. Throwing abandons the entire threads tier
        // when 7 of 8 workers had booted — and pay a full ~125 MiB re-download
        // to fall back to relaxed. rust/urlcodec/src/lib.rs's `codec_init`
        ...
        // workers that actually handshook, because participation is by id.

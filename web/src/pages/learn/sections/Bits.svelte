...
<Section id="bits">
  <p>
    ...
    to do with any particular model:
  </p>
  <ul>
  ...
  <p>
    Why −log₂? Because a bit is a <b>halving</b>: one yes/no question can at best cut the
    possibilities in half, so pinning down an event of probability <i>p</i> takes as many questions
    as there are halvings between 1 and <i>p</i>. The 1-in-256 case is literally binary search — "top
    half or bottom half?", eight times. The logarithm is also the only choice that makes bills add:
  ...
  <p>
    Compression is nothing more than paying the true bill. Ordinary storage pays 8 bits per character
    however obvious it was; a compressor pays −log₂ p and pockets the difference. Better
    probabilities, smaller total — which is the whole reason a <i>prediction machine</i> sits at the
    heart of a compressor: <b>compression and prediction are the same problem</b>.
  </p>
  <details class="surface mathbox">
    ...
    <div class="eq">
      ...
      <div class="c">
        ...
        the model and improving the compressor are one act.
      </div>
      <div class="c">
        <b>Symbols:</b> <i>p</i> — a probability, between 0 and 1 · log₂ — base-2 logarithm ("how many
        doublings") · <span class="mv">∑</span><sub><i>i</i></sub> — add up, once per token

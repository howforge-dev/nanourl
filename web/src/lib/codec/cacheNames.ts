...
// `nanourl-<8-hex artifact sha>`) and the offline shell precache (owned by
// src/sw.ts, named `nanourl-shell-<8-hex build id>`, scripts/sw-manifest.ts).
// Centralized here, rather than the `'nanourl-shell-'` prefix being
// duplicated as a literal string in sw.ts, loader.ts and the E2E, so a

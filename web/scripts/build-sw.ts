...
// devDependency — no workbox, no new dependency) rather than the main `vite
// build` in package.json's `build` script, which is configured for the page
// entries (rollupOptions.input, code-split ES modules) and would be the
// wrong shape for a single self-contained worker file.
//
...
await build({
  ...
  build: {
    ...
    lib: {
      ...
      formats: ['iife'],
      // Unused at runtime (sw.ts has no exports) — rollup's iife format

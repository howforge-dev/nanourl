 ...
 * precache (`nanourl-shell-<build>`) to hold every file self.__SHELL__
 * lists, by fetching dist/sw.js's own injected list (scripts/sw-manifest.ts)
 * and polling `caches.keys()` + that cache's own `.keys().length` against
 * it. */
...
test('compressor page works fully offline after one visit', async ({ page, context }) => {
  ...
  // artifact here — the network really is off, at the browser/CDP level ---

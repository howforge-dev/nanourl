...
// iife format, with no `{ type: 'module' }`, so `register('/sw.js')` from
// every page's registerSw.ts works with no extra options).
// scripts/sw-manifest.ts then injects `self.__SHELL__` (every dist/ shell
// file, site-absolute) and `self.__BUILD__` (a short sha of that list) as two
// plain assignments prepended ahead of this bundle's own code; see that
// script for how the list and id are derived (unit tested against a fixture
// rather than a real dist/).
//
// Update flow: install never calls self.skipWaiting(). A newly installed
// worker parks itself as `registration.waiting` and the standard lifecycle
// applies: it only activates once every client of the *previous* worker has
// naturally dropped off, so a background deploy can never yank an open tab
// out from under whatever the visitor is doing mid-encode. The only other
// path to activation is explicit: BuildInfo.svelte's "reload" button posts
...
// only *after* that (once this worker controls every client that
...
self.addEventListener('activate', (event) => {
  event.waitUntil(
    (async () => {
      // Claim first: only once this worker controls every open client is it
      // safe to delete another shell cache; otherwise some other
      // still-controlling worker instance could be mid-fetch against the
      // entry this is about to remove.
...
// network and failing outright once offline. This project owns every entry
// in this cache outright (install populated it from this exact build's own
// file list) and never wants content negotiation on it, so ignoring Vary
// here is correct.

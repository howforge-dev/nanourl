...
<Section id="lm" n={5} title="What a language model actually is">
  ...
  <p>
    <b>Training</b> is the answer, and it is mechanical: show the machine an example, measure how
    ...
    (<b>backpropagation</b>) gives the right nudge for <i>all</i> {fmtCount(numbers.params)} dials at
    ...
    because it reduced errors. "<b>Parameters</b>" and "<b>weights</b>" are these dials.
  </p>
  <h3>The contract</h3>
  <p>Strip away the mystique and a language model is a neural network with one specific job:</p>
  <Callout>
    Given the tokens so far, output a probability for <b>every one</b> of the
  ...
  <p>
    ...
    <i>advanced</i>, and click any token — the table shows the model's actual ranked guesses at that
    moment.
  </p>
</Section>

 ...
 * writes one to dist as a real file (today it doesn't — Vite's JSON import
 * inlines src/lib/assets.json straight into the JS bundles, so there is no
 * dist/assets.json to precache — but this checks for it conditionally
 ...
 * filesystem itself, and returns paths sorted and prefixed with `/` — sorted
 * so the build id derived from this list (buildId, below) doesn't depend on

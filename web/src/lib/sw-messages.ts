...
// handler and BuildInfo.svelte's "reload" button, split out (same pattern as
// sw-routes.ts) so vitest can exercise the predicate without a real
// ServiceWorkerGlobalScope. See tests/sw-messages.test.ts.
//
// A new service worker never calls self.skipWaiting() on its own: it parks
// itself as `registration.waiting` until every client of the previous worker

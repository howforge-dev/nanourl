 ...
 *  - `/sw.js` itself — precaching the worker's own script would hide its own
 *    updates from the browser's update check, which depends on always
 ...
 *  - cross-origin requests — this worker's scope is same-origin only, but a
 *    request built by hand (as the unit tests do) can still carry a foreign
 *    origin, so this checks explicitly rather than assuming the caller
 *    already filtered it.
 */
